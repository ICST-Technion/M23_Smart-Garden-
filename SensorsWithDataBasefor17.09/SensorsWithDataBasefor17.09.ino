...
const char* WIFI_PASSWORD = "N0aaaaaa";
...
void loop()
{
  ...
  soil_moisture_percent = ( 100 - ( (soil_moisture/4095.00) * 100 ) ); //switching from the  returned value from the capacitive soil mooisture to a percentage
  ...
  if(exec==0){
    ...
    if(Firebase.getInt(fbdo, "/Robophone/5669122872442880/start_irrigation")){
      manual_start_irrigation=fbdo.to<int>();
       //Serial.printf("##############1", manual_start_irrigation );
    }else{
      Serial.printf(fbdo.errorReason().c_str());
      Serial.println(); 
    }
    if(manual_start_irrigation == 1){ // it means that the plant need more water
     ...
     // Serial.printf("##############2", manual_start_irrigation );

    //update the calibration values if we need to update
    
    if(exec != prev_exec){
      //Serial.printf(exec);
     // Serial.println(prev_exec);
      int tmp_too_wet_min, tmp_too_wet_max , tmp_wet_min,tmp_wet_max,tmp_meduim_min,tmp_meduim_max,tmp_dry_min,tmp_dry_max;
      exec=0;
      while(exec == 0){ //means one or more reading from the DB has failed so try againg, after that we can keep moving
        exec=1;
